...
class Dict:
    def __init__(self,dictObj):
        Dict.dict_to_object(dictObj,self)

    @staticmethod
    def dict_to_object(dictObj,obj=None):
        if obj is not None:
            for k, v in dictObj.items():
                obj.__setattr__(k,Dict.dict_to_object(v))
        else:
            if isinstance(dictObj, dict):
                return Dict(dictObj)
            elif isinstance(dictObj, list):
                return [Dict.dict_to_object(v) for v in dictObj]
            else:
                return dictObj
