...
class KGModel(nn.Module, ABC):
    ...
    def __init__(self, sizes, rank, dropout, gamma, data_type, bias, init_size):
        ...
        self.sym_rel_ids = nn.Embedding(sizes[1], 1)

    # 对称关系id
    def set_sym_rel_ids(self, sym_rel_ids):
        tmp = torch.tensor(sym_rel_ids * 2 - 1., dtype=self.data_type).cuda()
        tmp = torch.cat([tmp, tmp], dim=0)
        self.sym_rel_ids.weight.data = tmp
        self.sym_rel_ids.weight.requires_grad = False
    ...
    def get_factors(self, queries):
        ...
        """
        head_e = self.entity(queries[:, 0].long())
        rel_e = self.rel(queries[:, 1].long())
        rhs_e = self.entity(queries[:, 2].long())
    ...
    def get_ranking(self, queries, filters, batch_size=1000):
        ...
        with torch.no_grad():
            ...
            while b_begin < N:
                ...
                #print(these_queries[0,2].item(), targets[0,0], scores[0, these_queries[0,2].item()])
                #print(torch.sum((torch.abs(scores - targets)<1e-9).float(), dim=1).cpu())
    ...
    def compute_metrics(self, examples, filters, batch_size=500):
        ...
        for m in ["rhs", "lhs"]:
            ...
            if m == "lhs":
                ...
                q[:, 2] = tmp
                q[:, 1] += self.sizes[1] // 2
            ranks = self.get_ranking(q, filters[m], batch_size=50)
